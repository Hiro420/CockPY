...
function action_EVENT_SELECT_OPTION_2003(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240504003, suite = 2 }) then
			return -1
		end
...
function action_EVENT_SELECT_OPTION_2004(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240504003, suite = 5 }) then
			return -1
		end
	
	-- 删除指定group： 240504002 ；指定config：2001；物件身上指定option：12；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 240504002, 2001, 12) then
		return -1
	end
