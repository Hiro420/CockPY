...
function action_EVENT_SELECT_OPTION_1(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240102003, suite = 2 }) then
			return -1
		end
...
function action_EVENT_SELECT_OPTION_17(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240102002, suite = 2 }) then
			return -1
		end
	
	-- 删除指定group： 240102001 ；指定config：1002；物件身上指定option：12；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 240102001, 1002, 12) then
		return -1
	end
