...
-- 装置
gadgets = {
	{ config_id = 17, gadget_id = 70900205, pos = { x = 7.2, y = -1.5, z = -7.3 }, rot = { x = 0.0, y = 0.0, z = 0.0 }, level = 1 }
}
...
function action_EVENT_ANY_MONSTER_LIVE_19(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 2, 164, 180, 240300004, 2, 0) then
		return -1
	end
	...
	-- 删除指定group： 240300002 ；指定config：1；物件身上指定option：12；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 240300002, 1, 12) then
		return -1
	end
...
function action_EVENT_CHALLENGE_FAIL_20(context, evt)
	...
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 240300002, 1, {12}) then
		return -1
	end
