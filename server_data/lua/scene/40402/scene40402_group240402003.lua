...
-- 装置
gadgets = {
	{ config_id = 25, gadget_id = 70900205, pos = { x = -8.4, y = -1.4, z = -5.6 }, rot = { x = 0.0, y = 0.0, z = 0.0 }, level = 1 }
}
...
function action_EVENT_ANY_MONSTER_LIVE_13(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 2, 161, 180, 240402003, 1, 0) then
		return -1
	end
	...
	-- 删除指定group： 240402002 ；指定config：5；物件身上指定option：12；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 240402002, 5, 12) then
		return -1
	end
...
function action_EVENT_CHALLENGE_FAIL_14(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240402003, suite = 2 }) then
			return -1
		end
	
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 240402002, 5, {12}) then
		return -1
	end
