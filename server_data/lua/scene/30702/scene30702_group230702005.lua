...
function action_EVENT_SELECT_OPTION_5003(context, evt)
	...
	if 0 ~= ScriptLib.AutoMonsterTide(context, 1, 230702006, {6001,6002,6003,6004}, 10, 2, 2) then
		return -1
	end
	
	-- 创建编号为2（该怪物潮的识别id)的怪物潮，创建怪物总数为10，场上怪物最少2只，最多2只
	if 0 ~= ScriptLib.AutoMonsterTide(context, 2, 230702006, {6009,6010,6011,6026}, 10, 2, 2) then
		return -1
	end
	
	-- 创建编号为3（该怪物潮的识别id)的怪物潮，创建怪物总数为5，场上怪物最少1只，最多1只
	if 0 ~= ScriptLib.AutoMonsterTide(context, 3, 230702006, {6006,6022}, 5, 1, 1) then
		return -1
	end
	
	-- 创建编号为4（该怪物潮的识别id)的怪物潮，创建怪物总数为5，场上怪物最少1只，最多1只
	if 0 ~= ScriptLib.AutoMonsterTide(context, 4, 230702006, {6005,6007}, 5, 1, 1) then
		return -1
	end
...
function action_EVENT_SELECT_OPTION_5004(context, evt)
	...
	if 0 ~= ScriptLib.AutoMonsterTide(context, 1, 230702007, {7001,7008}, 5, 1, 1) then
		return -1
	end
	
	-- 创建编号为2（该怪物潮的识别id)的怪物潮，创建怪物总数为10，场上怪物最少2只，最多2只
	if 0 ~= ScriptLib.AutoMonsterTide(context, 2, 230702007, {7002,7015,7007,7016}, 10, 2, 2) then
		return -1
	end
	
	-- 创建编号为3（该怪物潮的识别id)的怪物潮，创建怪物总数为10，场上怪物最少2只，最多2只
	if 0 ~= ScriptLib.AutoMonsterTide(context, 3, 230702007, {7003,7004,7013,7014}, 10, 2, 2) then
		return -1
	end
	
	-- 创建编号为4（该怪物潮的识别id)的怪物潮，创建怪物总数为5，场上怪物最少1只，最多1只
	if 0 ~= ScriptLib.AutoMonsterTide(context, 4, 230702007, {7005,7006}, 5, 1, 1) then
		return -1
	end
