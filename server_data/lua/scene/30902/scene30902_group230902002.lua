...
function action_EVENT_SELECT_OPTION_2003(context, evt)
	...
	if 0 ~= ScriptLib.SetIsAllowUseSkill(context, 1) then
		...
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 230902003, suite = 2 }) then
			return -1
		end
	...
	-- 改变指定group组230902001中， configid为1002的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 230902001, 1002, GadgetState.Default) then
			return -1
		end 
	
	-- 改变指定group组230902001中， configid为1001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 230902001, 1001, GadgetState.Default) then
			return -1
		end 
