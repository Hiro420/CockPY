...
-- 怪物
monsters = {
	{ config_id = 3001, monster_id = 23020101, pos = { x = 0.2, y = 0.0, z = -8.1 }, rot = { x = 0.0, y = 346.5, z = 0.0 }, level = 1, affix = { 1221 }, isElite = true, pose_id = 0 }
}
...
function action_EVENT_ANY_MONSTER_LIVE_3031(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 1, 197, 600, 231204003, 1, 0) then
		return -1
	end
...
function condition_EVENT_ANY_MONSTER_DIE_3037(context, evt)
	...
	if ScriptLib.GetGroupMonsterCountByGroupId(context, 231204003) ~= 0 then
		return false
	end
...
function action_EVENT_ANY_MONSTER_DIE_3037(context, evt)
	-- 添加suite6的新内容
	    ScriptLib.AddExtraGroupSuite(context, 231204003, 6)
